...
public static class ResiliencePipelineExtensions
{
    ...
    {
        public TBuilder AddSqlServerResiliencePipeline()
        {
            builder.Services.AddResiliencePipeline(
                ResiliencePipelineNames.SqlServerRetry,
                (pipelineBuilder, context) =>
                {
                    pipelineBuilder.AddRetry(new RetryStrategyOptions
                    {
                        ShouldHandle = new PredicateBuilder()
                            ...
                            .HandleInner<Win32Exception>(ShouldRetryOn)
                        ...
                        OnRetry = args =>
                        {
                            context.ServiceProvider.GetService<ILogger>()?
                                .LogWarning("{Pipeline} retry pipeline retry {Attempt} with delay {Delay}ms. Outcome type is {OutcomeType}.",
                                    ResiliencePipelineNames.SqlServerRetry,
                                    args.AttemptNumber,
    ...
    internal static bool ShouldRetryOnSqlException(SqlException ex)
    {
    ...
    internal static bool ShouldRetryOn(Win32Exception ex) =>
